...
public class ShortenedReferenceProcessDeployer extends AbstractProcessDeployer {
	...
	public void initializeNodes() {
		...
		Task task = TaskFactory.createRoleTask(
				"Positionen auf Anspruch pr�fen",
				"Anspruchspositionen �berpr�fen", objectionClerk);
		...
		task = TaskFactory.createRoleTask("Abhilfebescheid pr�fen",
				"Pr�fen des Abhilfebescheids", objectionClerk);
		Node human2 = builder
		...
		task = TaskFactory.createRoleTask("Widerspruch pr�fen",
				"Widerspruch erneut pr�fen auf neue Anspr�che", objectionClerk);
		...
		task = TaskFactory
				.createRoleTask("neues Gutachten erstellen",
						"Anspruchspunkte in neues Gutachten �bertragen",
						objectionClerk);
		Node human4 = builder
				.createNode(createParamBuilderFor(HumanTaskActivity.class,
						Task.class, task).buildNodeParameter());
		...
		task = TaskFactory.createRoleTask("Nachbearbeitung",
				"abschlie�ende Nachbearbeitung des Falls", objectionClerk);
		Node human5 = builder
				.createNode(createParamBuilderFor(HumanTaskActivity.class,
						Task.class, task).buildNodeParameter());

		// human task for allowance clerk, task is to enforce allowance
		task = TaskFactory.createRoleTask("Leistungsgew�hrung umsetzen",
				"Leistungsanspr�che durchsetzen", allowanceClerk);
		Node human6 = builder
				.createNode(createParamBuilderFor(HumanTaskActivity.class,
						Task.class, task).buildNodeParameter());
