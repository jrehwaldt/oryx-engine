...
public class BpmnXmlParse extends XmlParse {
    ...
    // protected Map<String, BpmnInterfaceImplementation> interfaceImplementations = new HashMap<String,
    // BpmnInterfaceImplementation>();
    // protected Map<String, OperationImplementation> operationImplementations = new HashMap<String,
    ...
    // public BpmnXmlParse deployment(DeploymentEntity deployment) {
    //
    ...
    //
    // List<Element> imports = rootElement.getElements("import");
    // for (Element theImport : imports) {
    // String importType = theImport.getAttribute("importType");
    ...
    // try {
    // wsdlImporterClass = Class.forName("org.activiti.engine.impl.webservice.WSDLImporter", true, Thread
    ...
    //
    // /**
    // * Parses the itemDefinitions of the given definitions file. Item definitions are not contained within a process
    // * element, but they can be referenced from inner process elements.
    // *
    // */
    // public void parseItemDefinitions() {
    //
    // for (Element itemDefinitionElement : rootElement.getElements("itemDefinition")) {
    ...
    //
    // /**
    // * Parses the messages of the given definitions file. Messages are not contained within a process element, but
    // they
    // * can be referenced from inner process elements.
    // *
...
//                String startFormHandlerClassName = startEventElement.attributeNS(
...
//
//                String initiatorVariableName = startEventElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
//                    "initiator");
...
//            }
    ...
    // if (isSequential) {
    // miActivityBehavior = new SequentialMultiInstanceBehavior(activity,
    // (AbstractBpmnActivityBehavior) activity.getActivityBehavior());
    // } else {
    // miActivityBehavior = new ParallelMultiInstanceBehavior(activity,
    // (AbstractBpmnActivityBehavior) activity.getActivityBehavior());
    // }
    ...
    // miActivityBehavior.setCompletionConditionExpression(expressionManager
    ...
    // miActivityBehavior.setCollectionExpression(expressionManager
    ...
    // String elementVariable = miLoopCharacteristics.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "elementVariable");
    ...
    // }
    //
    // resultVariableName = scriptTaskElement
    // .attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "resultVariable");
    // if (resultVariableName == null) {
    // // for backwards compatible reasons
    // resultVariableName = scriptTaskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "resultVariableName");
    // }
    // }
    //
    ...
    // "resultVariable");
    // if (resultVariableName == null) {
    // resultVariableName = serviceTaskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "resultVariableName");
    // }
    ...
    //
    // String ruleInputString = businessRuleTaskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "ruleVariablesInput");
    ...
    // String resultVariableNameString = businessRuleTaskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "resultVariable");
    // if (resultVariableNameString == null) {
    // resultVariableNameString = businessRuleTaskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "resultVariableName");
    // }
    //
    ...
    // ruleActivity
    // .addRuleVariableInputIdExpression(expressionManager.createExpression(ruleInputObject.trim()));
    // }
    // }
    //
    // if (rulesString != null) {
    ...
    // } else {
    // Expression transformation = this.expressionManager.createExpression(dataAssociationElement.element(
    // "transformation").getText());
    // AbstractDataAssociation dataOutputAssociation = new TransformationDataOutputAssociation(null, targetRef,
    // transformation);
    ...
    // activity.setActivityBehavior((MailActivityBehavior) ClassDelegate.instantiateDelegate(
    ...
    // }
    // List<Element> fieldDeclarationElements = elementWithFieldInjections.elementsNS(
    // BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS, "field");
    ...
    //
    // FieldDeclaration fieldDeclaration = parseStringFieldDeclaration(fieldDeclarationElement, serviceTaskElement,
    // fieldName);
    // if (fieldDeclaration == null) {
    ...
    //
    // try {
    // String expression = getStringValueFromAttributeOrElement("expression", "expression",
    ...
    // addError("No valid value found in attribute '" + attributeName + "' nor element '" + elementName + "'",
    // element);
    // } else {
    // // Use text of element
    ...
    // String taskFormHandlerClassName = taskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "formHandlerClass");
    ...
    // } else {
    // addError(
    // "Invalid usage: duplicate assignee declaration for task " + taskDefinition.getNameExpression(),
    // taskElement);
    // }
    // }
    //
    // // Candidate users
    // String candidateUsersString = taskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    ...
    //
    // // Candidate groups
    // String candidateGroupsString = taskElement.attributeNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    ...
    //
    // /**
    // * Parses the given String as a list of comma separated entries, where an entry can possibly be an expression that
    // * has comma's.
    // *
    ...
    // List<Element> taskListenerElements = extentionsElement.elementsNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "taskListener");
    ...
    // *
    // * Note that the boundary events are not parsed during the parsing of the bpmn activities, since the semantics are
    // * different (boundaryEvent needs to be added as nested activity to the reference activity on PVM level).
    // *
    // * @param parentElement
    // * The 'parent' element that contains the activities (process, subprocess).
    // * @param scopeElement
    ...
    //
    // /**
    // * Parses a boundary timer event. The end-result will be that the given nested activity will get the appropriate
    // * {@link ActivityBehavior}.
    // *
    ...
    // * @param timerActivity
    // * The activity which maps to the structure of the timer event on the boundary of another activity. Note
    // * that this is NOT the activity onto which the boundary event is attached, but a nested activity inside
    // * this activity, specifically created for this event.
    // */
    ...
    // }
    //
    // private TimerDeclarationImpl parseTimer(Element timerEventDefinition, ScopeImpl timerActivity, String
    // jobHandlerType) {
    //
    // // TimeDate
    ...
    // ErrorEndEventActivityBehavior behavior = (ErrorEndEventActivityBehavior) errorEndEvent
    // .getActivityBehavior();
    // if (errorRef == null || errorRef.equals(behavior.getErrorCode())
    ...
    // catchingActivity = activity.getProcessDefinition()
    // .findActivity(behavior.getBorderEventActivityId());
    // }
    //
    ...
    //
    // for (BpmnParseListener parseListener : parseListeners) {
    // parseListener.parseBoundaryErrorEventDefinition(errorEventDefinition, interrupting, activity,
    ...
    //
    // /**
    // * Parses a subprocess (formely known as an embedded subprocess): a subprocess defined withing another process
    // * definition.
    // *
    ...
    // if (sourceExpression != null) {
    // Expression expression = expressionManager.createExpression(sourceExpression.trim());
    // callActivityBehaviour.addDataOutputAssociation(new MessageImplicitDataOutputAssociation(target,
    // expression));
    // } else {
    // String source = listenerElement.attribute("source");
    // callActivityBehaviour.addDataOutputAssociation(new MessageImplicitDataOutputAssociation(target,
    ...
    //
    // /**
    // * Parses the properties of an element (if any) that can contain properties (processes, activities, etc.)
    // *
    ...
    // if (name == null) {
    // if (id == null) {
    // addError("Invalid property usage on line " + propertyElement.getLine() + ": no id or name specified.",
    // propertyElement);
    // } else {
    // name = id;
    // }
    ...
    //
    // // Implicit check: sequence flow cannot cross (sub) process boundaries: we don't do a
    // // processDefinition.findActivity here
    ...
    // if (extentionsElement != null) {
    // List<Element> listenerElements = extentionsElement.elementsNS(BpmnParser.ACTIVITI_BPMN_EXTENSIONS_NS,
    // "executionListener");
    ...
    //
    // } else {
    // addError("Invalid reference in 'bpmnElement' attribute, process " + processId + " not found",
    // bpmnPlaneElement);
    // }
    // } else {
    ...
    // activity.setWidth(parseDoubleAttribute(bpmnShapeElement, "width", bounds.attribute("width"), true)
    // .intValue());
    // activity.setHeight(parseDoubleAttribute(bpmnShapeElement, "height", bounds.attribute("height"),
    // true).intValue());
    // } else {
    ...
    // }
    // } else {
    // addError("Invalid reference in 'bpmnElement' attribute, activity " + activityId + "not found",
    // bpmnShapeElement);
    // }
    // } else {
    ...
    // waypoints.add(parseDoubleAttribute(waypointElement, "x", waypointElement.attribute("x"), true)
    // .intValue());
    // waypoints.add(parseDoubleAttribute(waypointElement, "y", waypointElement.attribute("y"), true)
    // .intValue());
    // }
    ...
    // }
    // } else {
    // addError("Invalid reference in 'bpmnElement' attribute, sequenceFlow " + sequenceFlowId + "not found",
    // bpmnEdgeElement);
    // }
    // } else {
